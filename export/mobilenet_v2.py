...
WEIGHT_PATH = "model_data/deeplab_mobilenetv2.wts"
ENGINE_PATH = "model_data/deeplab_mobilenetv2.engine"
...
def conv_bn_relu(network, weight_map, input, outch, ksize, s, g, lname):
    ...
    conv1 = network.add_convolution(input=input,
                                    ...
                                    kernel=weight_map[lname + "weight"],
...
def conv_bn_relu1(network, weight_map, input, outch, ksize, s, g, lname):
    p = (ksize - 1) // 2
...
# "backbone.features.1."
# 'backbone.features.1.conv.0.weight'
...
def inverted_res(network, weight_map, input, lname, inch, outch, s, exp):
    ...
    if exp != 1:
        ...
        conv1 = network.add_convolution(input=ew1.get_output(0),
                                        ...
                                        bias=trt.Weights())
        assert conv1
        bn1 = add_batch_norm_2d(network, weight_map, conv1.get_output(0), lname + "conv.1", EPS)
...
def create_engine(max_batch_size, builder, config, dt):
    ...
    ew1 = conv_bn_relu1(network, weight_map, data, 32, 3, 2, 1, "backbone.features.0.")
    ir1 = inverted_res(network, weight_map, ew1.get_output(0), "backbone.features.1.", 32, 16, 1, 1)
    ir1 = inverted_res(network, weight_map, ir1.get_output(0), "backbone.features.2.", 16, 24, 2, 6)
    ir1 = inverted_res(network, weight_map, ir1.get_output(0), "backbone.features.3.", 24, 24, 1, 6)
    ir1 = inverted_res(network, weight_map, ir1.get_output(0), "backbone.features.4.", 24, 32, 2, 6)
    ir1 = inverted_res(network, weight_map, ir1.get_output(0), "backbone.features.5.", 32, 32, 1, 6)
    ir1 = inverted_res(network, weight_map, ir1.get_output(0), "backbone.features.6.", 32, 32, 1, 6)
    ir1 = inverted_res(network, weight_map, ir1.get_output(0), "backbone.features.7.", 32, 64, 2, 6)
    ir1 = inverted_res(network, weight_map, ir1.get_output(0), "backbone.features.8.", 64, 64, 1, 6)
    ir1 = inverted_res(network, weight_map, ir1.get_output(0), "backbone.features.9.", 64, 64, 1, 6)
    ir1 = inverted_res(network, weight_map, ir1.get_output(0), "backbone.features.10.", 64, 64, 1, 6)
    ir1 = inverted_res(network, weight_map, ir1.get_output(0), "backbone.features.11.", 64, 96, 1, 6)
    ir1 = inverted_res(network, weight_map, ir1.get_output(0), "backbone.features.12.", 96, 96, 1, 6)
    ir1 = inverted_res(network, weight_map, ir1.get_output(0), "backbone.features.13.", 96, 96, 1, 6)
    ir1 = inverted_res(network, weight_map, ir1.get_output(0), "backbone.features.14.", 96, 160, 2, 6)
    ir1 = inverted_res(network, weight_map, ir1.get_output(0), "backbone.features.15.", 160, 160, 1, 6)
    ir1 = inverted_res(network, weight_map, ir1.get_output(0), "backbone.features.16.", 160, 160, 1, 6)
    ir1 = inverted_res(network, weight_map, ir1.get_output(0), "backbone.features.17.", 160, 320, 1, 6)
    ew2 = conv_bn_relu(network, weight_map, ir1.get_output(0), 1280, 1, 1, 1, "backbone.features.18.")
